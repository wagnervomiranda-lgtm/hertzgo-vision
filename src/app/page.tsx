...
const MSG_DEFAULT: Mensagens = {
  ...
  msg2b_parkway: "[nome], você é cliente frequente no Park Way! 🙏\n\nCupom fidelidade: [cupom]",
...
function TabDashboard({sessions,meta,onMetaChange}:{sessions:Session[];meta:number;onMetaChange:(v:number)=>void}){
  ...
  const dayData=Object.values(byDay).sort((a,b)=>a.date.getTime()-b.date.getTime()).map(d=>({date:fmtDate(d.date),rev:+d.rev.toFixed(2),kwh:+d.kwh.toFixed(0)}));
    ...
    <div style={{padding:"24px 28px"}}>
      ...
      {dcSess.length>0&&acSess.length>0&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:28}}>
          {[{label:"⚡ DC 120kW",sess:dcSess,rev:dcRev,kwh:dcKwh,color:"#a78bfa",bg:"rgba(139,92,246,0.08)",border:"rgba(139,92,246,0.25)"},{label:"🔌 AC 22kW",sess:acSess,rev:acRev,kwh:acKwh,color:T.green,bg:"rgba(0,229,160,0.08)",border:"rgba(0,229,160,0.25)"}].map((dc,i)=>(
        ...
        <>
          ...
          <div style={{background:"rgba(59,130,246,0.06)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:12,padding:"16px 20px",marginBottom:16}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
              {[{status:"🟠 Em Risco",score:"26–50",acao:"AGIR AGORA — frequência caindo. Enviar MSG de reengajamento.",cor:"#fb923c"},{status:"🔴 Churn",score:"0–25",acao:"Última tentativa de resgate. Após isso, arquivar.",cor:T.red},{status:"🟡 Regular",score:"51–75",acao:"Monitorar. Enviar novidade quando tiver.",cor:T.amber},{status:"🟢 VIP Ativo",score:"76–100",acao:"Reconhecer — não interromper. MSG VIP para fidelizar mais.",cor:T.green}].map((s,i)=>(
            ...
            <div style={{fontFamily:T.mono,fontSize:10,color:T.text3,marginTop:10}}>Critérios: recargou nos últimos 7 dias (+40pts) · 2+ recargas/semana (+35pts) · cliente há 30+ dias (+25pts)</div>
          </div>
      ...
      {activeSection==="contatos"&&(
        <>
          <div style={{background:"rgba(59,130,246,0.06)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:10,padding:"12px 16px",fontFamily:T.mono,fontSize:11,color:"#93c5fd",marginBottom:20}}>
            ℹ️ Importe o CSV de usuários de qualquer estação. A estação é detectada automaticamente pelo nome do arquivo ou conteúdo. Faça 1x por semana.
          </div>
          <div style={{background:T.bg2,border:`1px solid ${T.border}`,borderRadius:14,padding:"24px",marginBottom:20,textAlign:"center"}}>
            ...
            <div style={{fontFamily:T.mono,fontSize:11,color:T.text2,marginBottom:20}}>A estação é detectada automaticamente</div>
            ...
            {([{id:"instanceId" as keyof ZAPIConfig,label:"ID da Instância",placeholder:"Ex: 3DF217DC18D...",desc:"Z-API → Instâncias → ID"},{id:"token" as keyof ZAPIConfig,label:"Token da Instância",placeholder:"Token...",desc:"Z-API → Instâncias → Token"},{id:"clientToken" as keyof ZAPIConfig,label:"Client-Token *obrigatório",placeholder:"Client-Token...",desc:"Z-API → Conta → Security"}] as {id:keyof ZAPIConfig;label:string;placeholder:string;desc:string}[]).map(f=>(
              <div key={f.id}>
                ...
                <input type={f.id==="instanceId"?"text":"password"} value={zapi[f.id]} placeholder={f.placeholder}
                  ...
                  style={{width:"100%",background:T.bg3,border:`1px solid ${T.border}`,color:T.text,padding:"8px 10px",borderRadius:8,fontSize:12,fontFamily:T.mono}}/>
