...
interface UserOverride {
  ...
  precoVip?: number; // preço VIP cadastrado na Spott para este usuário
  respostaMsg1?: "1"|"2"; // resposta registrada manualmente
  respostaEm?: string; // data ISO da resposta
...
interface AppState {
  ...
  operadoresIgnorar: string[]; // nomes de usuários operadores (ex: Wagner Miranda)
...
const ESTACAO_TIPO: Record<string, "propria" | "parceria" | "contratual"> = {
  costa: "propria", parkway: "propria", cidadeauto: "propria",
...
const ESTACAO_PROPRIA = ["parkway", "cidadeauto", "costa"]; // costa é própria (split financeiro só no DRE)
const ESTACAO_SPLIT   = ["costa"]; // estações com split de sócio — usada apenas no DRE
const ESTACAO_PARCERIA: string[] = []; // nenhuma estação é apenas parceria agora
...
function calcHealthScore(sessions:Session[],cfg:DREConfig|null,hubK:string):HealthScore{
  ...
  // Se tem usina solar, threshold de preço é menor (custo real é ~R$0,04)
  const isSolar = cfg?.solarProprio === true;
  if(priceKwh>=(isSolar?0.80:1.30))op+=8;else if(priceKwh>=(isSolar?0.50:1.00))op+=4;
...
function getSegmento(
  ...
  // Parceiro: paga grátis OU preço muito abaixo da média da estação (85% do preço praticado)
  const estKey = uSess[0]?.hubKey || "";
  const precoPraticado = calcPrecoPraticado(sessoes, estKey);
  const thresholdParceiro = precoPraticado > 0 ? precoPraticado * 0.85 : 1.00;
  if (temGratis || (mediaKwh > 0 && mediaKwh < thresholdParceiro)) return { segmento: "Parceiro", prioridade: 1, cor: "#3b82f6", fonte: "csv" };
...
function gerarFilaDia(
  ...
  Object.entries(userMap).forEach(([nome, data]) => {
    ...
    // Ignorar operadores (Wagner Miranda, etc)
    const operadores = appState.operadoresIgnorar || ["wagner miranda","wagner vinícius","executivo"];
    if (operadores.some(op => nome.toLowerCase().includes(op))) return;
  ...
  const manuaisHoje = appState.disparosManuaisHoje || 0;
  const slotsRestantes = Math.max(0, limite - disparadosHoje - manuaisHoje);
...
function classificarUsuarios(sessions:Session[]):UserData[]{
  ...
  return Object.values(userMap).map(u=>{
    ...
    const localKey=Object.entries(stCount).sort((a,b)=>b[1]-a[1])[0]?.[0]||"";
    const precoPrat=calcPrecoPraticado(sessions,localKey);
    const threshParceiro=precoPrat>0?precoPrat*0.85:1.00;
    const isParceiro=temGratis||(mediaKwh>0&&mediaKwh<threshParceiro);
...
function TabDRE({sessions,appState}:{sessions:Session[];appState:AppState}){
  ...
  if(cfg){
    ...
    if(cfg.pctEspaco>0)dreRows.push({label:`(−) ${cfg.modelo==="investidor"&&cfg.invNome?cfg.invNome.split(" ")[0]+" (split)":("Parceiro")} (${cfg.pctEspaco}%)`,val:-custoEspaco});
...
function FilaDoDia({sessions,appState,localDisparos,getMsgTemplate,abrirPreview,enviarUm,onSaveState,isMobile,sending}:FilaDoDiaProps){
  const limite=appState.limiteDisparoDiario||30;
...
function TabAcoes({sessions,appState,onSaveDisparos,onSaveState}:{sessions:Session[];appState:AppState;onSaveDisparos:(d:AppState["disparos"])=>void;onSaveState:(p:Partial<AppState>)=>void}){
  ...
  const enviarUm=async(user:string,hubK:string,msgId:string,template:string,cupom:string="")=>{
    ...
    const ov3=appState.userOverrides[user.toLowerCase()];
    const msg=montarMsg(template,user,hubK,cupom,ov3?.precoVip);
...
Intervalo: ${appState.metas["crm_intervalo_min"]||15}–${appState.metas["crm_intervalo_max"]||45} min entre cada`,qtd:elegiveisLimitados.length,onConfirm:()=>{setConfirmModal(null);resolve();}});
  ...
  const motoristasMigracao=users.filter(u=>u.isMotorista&&isCrmAtiva(u.localFreqKey)&&["mamute","madeiro_sia"].includes(u.localFreqKey)&&!jaContatado(u.user,"msg2a",60));
  const fidelizacao=users.filter(u=>!u.isParceiro&&!u.isMotorista&&ESTACAO_PROPRIA.includes(u.localFreqKey)&&!jaContatado(u.user,"msg2b",60));
  ...
  // ── FILA DOS 30 ─────────────────────────────────────────────────────
  const limite30=appState.limiteDisparoDiario||30;
  const {fila:filaDoDia,semTelefone:semTel30,novosDetectados:novosHoje}=useMemo(()=>gerarFilaDia(sessions,appState,limite30),[sessions,appState,limite30]);

  // ── ESTADO REGISTRO MANUAL ──────────────────────────────────────────
  const[regBusca,setRegBusca]=useState("");
  const[regResp,setRegResp]=useState<"1"|"2"|"">("");
  const[regPreco,setRegPreco]=useState("");
  const[regMsg,setRegMsg]=useState<string|null>(null);

  const registrarManual=()=>{
    if(!regBusca||!regResp){setRegMsg("Preencha nome e resposta.");return;}
    const key=regBusca.trim().toLowerCase();
    const novoOv:UserOverride={
      isMotorista:regResp==="1",
      respostaMsg1:regResp as "1"|"2",
      respostaEm:new Date().toISOString(),
      precoVip:regPreco?parseFloat(regPreco.replace(",",".")):undefined,
      atualizadoEm:new Date().toISOString(),
      fonte:"manual",
    };
    const novoDisparo={ts:new Date().toISOString(),nome:regBusca.trim(),msgId:"msg1_respondeu",status:"ok" as const,msg:`Resposta ${regResp} registrada manualmente`};
    onSaveState({userOverrides:{...appState.userOverrides,[key]:novoOv}});
    onSaveDisparos([...localDisparos,novoDisparo]);
    setLocalDisparos(p=>[...p,novoDisparo]);
    setRegMsg(`✅ ${regBusca.trim().split(" ")[0]} registrado — ${regResp==="1"?"Motorista":"Não motorista"}${regPreco?` · VIP R$${regPreco}/kWh`:""}`);
    setRegBusca("");setRegResp("");setRegPreco("");
    setTimeout(()=>setRegMsg(null),4000);
  };

  // ── CONTADOR DE MANUAIS ─────────────────────────────────────────────
  const manuaisHoje=appState.disparosManuaisHoje||0;
  const incrementarManual=()=>{onSaveState({disparosManuaisHoje:manuaisHoje+1});showToast(`Manual: ${manuaisHoje+1}/${limite30} hoje`);};
  const resetarManuais=()=>{onSaveState({disparosManuaisHoje:0});showToast("Contador zerado.");};

  // ── PAINEL AÇÃO 19/03 ────────────────────────────────────────────────
  // Painel ação: pegar data do primeiro disparo registrado, tratar como campanha
  const primeiroDisparo=localDisparos.filter(d=>d.status==="ok").sort((a,b)=>new Date(a.ts).getTime()-new Date(b.ts).getTime())[0];
  const dataCorteAcao=primeiroDisparo?new Date(new Date(primeiroDisparo.ts).getTime()+24*60*60*1000*2):new Date("2026-03-21T03:00:00Z");
  const labelAcao=primeiroDisparo?new Date(primeiroDisparo.ts).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit"}):"—";
  const contatadosAcao=localDisparos.filter(d=>d.status==="ok"&&new Date(d.ts)<=dataCorteAcao);
  const voltaramAcao=contatadosAcao.filter(d=>{
    const tsD=new Date(d.ts).getTime();
    return ok.some(s=>s.user===d.nome&&s.date.getTime()>tsD);
  });
  const pagandoVip=contatadosAcao.filter(d=>{
    const ov=appState.userOverrides[d.nome.toLowerCase()];
    if(!ov?.precoVip)return false;
    const sessRecentes=ok.filter(s=>s.user===d.nome&&s.energy>0&&s.value>0).slice(-3);
    if(!sessRecentes.length)return false;
    const precoRecente=sessRecentes.reduce((a,s)=>a+s.value,0)/sessRecentes.reduce((a,s)=>a+s.energy,0);
...
function TabConfig({appState,onSave}:{appState:AppState;onSave:(partial:Partial<AppState>)=>void}){
  ...
  const[limiteDisp,setLimiteDisp]=useState(appState.limiteDisparoDiario||30);
  ...
  const estacoesDRE=[
    {key:"costa",nome:"Costa",tipo:"Própria (split)"},{key:"parkway",nome:"Park Way",tipo:"Própria"},
...
function gerarRelatorioOpV2(sessions:Session[],appState:AppState,station:string):string{
  ...
  return`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Relatório Operacional — ${hubNome(station)}</title><style>${FLBR_CSS}</style></head><body><button class="print-btn" onclick="window.print()">🖨️ Imprimir</button>${cover}${demandaSection}${usersSection}<footer class="footer"><div class="footer-left">HertzGo · CNPJ 27.713.035/0001-24 · Brasília · DF<br>Gerado em ${new Date().toLocaleString("pt-BR")}</div><div class="footer-right">Uso Interno — Equipe Operações<br>${hubNome(station)} · Vision v5.3</div></footer><script>document.querySelectorAll('.logo-block img').forEach(function(img){img.onerror=function(){this.style.display='none';var span=document.createElement('span');span.style.cssText='font-family:var(--font);font-size:22px;font-weight:800;color:#e8f0f8;letter-spacing:-.02em;';span.innerHTML='Hertz<span style="color:var(--green)">Go</span>';this.parentNode.insertBefore(span,this);};});</script></body></html>`;
}
...
function gerarApresentacaoV2(sessions:Session[],appState:AppState):string{
  ...
  <div class="section">
    ...
    ${hubs.map(hub=>{
      ...
      const tipoRaw=(ESTACAO_TIPO as Record<string,string>)[hub]||"contratual";
      const tipoLabelRel=tipoRaw==="propria"?"Própria":tipoRaw==="parceria"?"Parceria":"Contratual";
      return`<div class="station-row"><div class="sr-score" style="color:${color}">${hs.total}</div><div style="flex:1"><div style="font-size:15px;font-weight:700;margin-bottom:4px">${hubNome(hub)}</div><div style="font-size:11px;color:var(--text2);font-family:var(--mono)">${hubOk.length} sessões · ${brlFmt(hubRev)} · ${tipoLabelRel}</div></div><div class="sr-bar"><div class="sr-fill" style="width:${hs.total}%;background:${color}"></div></div></div>`;
    }).join("")}
  </div>`;
  const gestaoSection=`
  <div class="section">
    <div class="section-header">
      ...
      <span class="section-tag tag-teal">Vision v5.3</span>
    </div>
  ...
  return`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pitch HertzGo — Rede de Eletropostos</title><style>${FLBR_CSS}</style></head><body><button class="print-btn" onclick="window.print()">🖨️ Imprimir</button>${cover}${mercadoSection}${redeSection}${gestaoSection}${ctaSection}<footer class="footer"><div class="footer-left">HertzGo · CNPJ 27.713.035/0001-24 · Brasília · DF<br>Gerado em ${new Date().toLocaleString("pt-BR")}</div><div class="footer-right">Confidencial · Investidores & Parceiros<br>Vision v5.3 · (61) 99803-7361</div></footer><script>document.querySelectorAll('.logo-block img').forEach(function(img){img.onerror=function(){this.style.display='none';var span=document.createElement('span');span.style.cssText='font-family:var(--font);font-size:22px;font-weight:800;color:#e8f0f8;letter-spacing:-.02em;';span.innerHTML='Hertz<span style="color:var(--green)">Go</span>';this.parentNode.insertBefore(span,this);};});</script></body></html>`;
}
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",paddingBottom:4,WebkitOverflowScrolling:"touch"}}>
          {hubs.map(h=>{
            ...
            return(<div key={h} onClick={()=>setStation(h)} style={{background:station===h?`${color}10`:T.bg2,border:`1px solid ${station===h?color+"60":T.border}`,borderRadius:12,padding:"10px 14px",cursor:"pointer",flexShrink:0,minWidth:120,position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontFamily:T.mono,fontSize:9,color:T.text3}}>{((t:string)=>t==="propria"?"Própria":t==="parceria"?"Parceria":"Contratual")((ESTACAO_TIPO as Record<string,string>)[h]||"contratual")}</span><span style={{fontFamily:T.mono,fontSize:14,fontWeight:800,color}}>{hs2.total}</span></div>
            </div>);
          })}
        </div>
      </div>
