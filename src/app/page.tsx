...
function calcAlertas(sessions: Session[]): { semaforo: "verde"|"amarelo"|"vermelho"; alertas: Alerta[] } {
  ...
  const semaforo = crits > 0 ? "vermelho" : warns >= 1 ? "amarelo" : "verde";
...
function ProjecaoMensal({ sessions, meta, onMetaChange }: { sessions: Session[]; meta: number; onMetaChange: (v: number) => void }) {
  ...
  const avgRevDay  = totalRev / days;
  const avgKwhDay  = totalKwh / days;
  const avgSessDay = totalSess / days;

  const diasNoMes = 30;
  const projRev  = avgRevDay * diasNoMes;
  const projKwh  = avgKwhDay * diasNoMes;
  const projSess = Math.round(avgSessDay * diasNoMes);

  // Dias restantes no mês real
  const hoje = new Date();
  const ultimoDia = new Date(hoje.getFullYear(), hoje.getMonth() + 1, 0).getDate();
  const diasRestantes = ultimoDia - hoje.getDate();
  const diasDecorridos = hoje.getDate();

  // Pacing: quanto já deveria ter feito até hoje vs o que foi feito
  ...
  // Insight copy estratégico
  const gerarInsight = (): string => {
    if (meta === 0) return "Configure uma meta mensal para ativar o pacing inteligente.";
    if (pctMeta >= 110) return `🚀 Ritmo excelente — projeção ${pctMeta.toFixed(0)}% da meta. Se mantiver, vai superar em R$\u00a0${(projRev - meta).toFixed(0)}.`;
    if (pctMeta >= 100) return `✅ No alvo — projeção bate a meta. Mantenha ${avgSessDay.toFixed(1)} sessões/dia.`;
    if (pctMeta >= 75) {
      if (ritmoDiff > 0) return `⚠️ Para bater a meta, você precisa de R$\u00a0${ritmoNecessario.toFixed(0)}/dia nos próximos ${diasRestantes} dias — R$\u00a0${ritmoDiff.toFixed(0)}/dia a mais que o ritmo atual.`;
      return `⚠️ Projeção em ${pctMeta.toFixed(0)}% da meta. Faltam R$\u00a0${faltaMeta.toFixed(0)} — ${diasRestantes} dias para recuperar.`;
    }
    return `🔴 Ritmo crítico — projeção em ${pctMeta.toFixed(0)}% da meta. Precisa de +${Math.ceil(ritmoDiff > 0 ? ritmoDiff / (avgRevDay / avgSessDay) : 2)} sessões/dia para reverter.`;
  };
    ...
    <div style={{ marginBottom: 24, background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"14px 20px", borderBottom:`1px solid ${T.border}` }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily:T.mono, fontSize:10, color:T.text2, marginTop:1 }}>base: {avgRevDay.toFixed(0)}/dia · {days} dias no CSV · {diasRestantes} dias restantes no mês</div>
          </div>
        </div>
        {/* Meta editável */}
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          {editando ? (
            <div style={{ display:"flex", gap:6 }}>
              <input autoFocus type="number" value={metaInput}
              ...
              <button onClick={() => { onMetaChange(+metaInput||0); setEditando(false); }}
                style={{ background:T.greenDim, border:`1px solid rgba(0,229,160,0.3)`, color:T.green, padding:"4px 10px", borderRadius:6, fontSize:11, cursor:"pointer", fontFamily:T.mono }}>✓</button>
            </div>
          ) : (
            <button onClick={() => { setMetaInput(String(meta)); setEditando(true); }}
              style={{ background:"transparent", border:`1px solid ${T.border}`, color: meta > 0 ? T.amber : T.text3, padding:"4px 12px", borderRadius:6, fontSize:12, cursor:"pointer", fontFamily:T.mono, transition:"all 0.2s" }}>
      ...
      {meta === 0 && (
        <div style={{ padding:"12px 20px", borderTop:`1px solid ${T.border}`, fontFamily:T.mono, fontSize:11, color:T.text3, textAlign:"center" }}>
          👆 Clique em <strong style={{ color:T.amber }}>Definir meta</strong> para ativar o pacing inteligente e ver se você está no ritmo certo
        </div>
      )}
    </div>
  );
    ...
    <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:48, minHeight:"100vh" }}>
      ...
      <div onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)}
        ...
        <div style={{ fontFamily:"'JetBrains Mono', monospace", fontSize:11, color:"#6b7fa3", lineHeight:1.8, marginBottom:24 }}>
          Arraste ou clique para selecionar<br/><span style={{ color:"#00e5a0" }}>Multi-estação · Qualquer período · Formato Spott</span>
        </div>
        ...
        <Panel>
          ...
          {cfg.modelo==="investidor"&&(<>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {inp("invNome","Nome Investidor","text")}{inp("invPct","% Investidor do LL","number")}
              {inp("invTotal","Investimento Total","number")}{inp("invPago","Já Investido","number")}
              {inp("invDividaPrio","Dívida Prioritária","number")}{inp("invAmort","Já Amortizado","number")}
            </div>
          </>)}
          {cfg.modelo==="propria"&&(<>
...
function TabAcoes({ sessions }: { sessions: Session[] }) {
  ...
  const enviar=async(user:string,nome:string,hub:string)=>{
    ...
    try{const r=await fetch("/api/zapi",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({phone:user,message:msg})});const d=await r.json();setLog(p=>[{ts:new Date().toLocaleTimeString("pt-BR"),nome,status:d.ok?"ok":"err",msg:d.erro},...p.slice(0,49)]);}
  ...
  return (
    <div style={{padding:"24px 28px"}}>
      ...
      <Panel style={{padding:0,overflow:"hidden"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          <thead><tr><th style={TH}>Perfil</th><th style={TH}>Usuário</th><th style={TH}>Hub</th><th style={THR}>kWh</th><th style={THR}>Receita</th><th style={THR}>Ação</th></tr></thead>
          <tbody>{leads.slice(0,50).map(u=>(
            <tr key={u.user} style={{borderBottom:"1px solid rgba(255,255,255,0.02)"}}>
              ...
              <td style={{...TDR,color:"#6b7fa3"}}>{u.kwh.toFixed(1)}</td>
              <td style={{...TDR,color:"#00e5a0",fontWeight:600}}>{brl(u.rev)}</td>
              <td style={TDR}><button onClick={()=>enviar(u.user,u.user.split(" ")[0],u.localFreq)} disabled={sending[u.user]} style={{padding:"4px 12px",borderRadius:6,fontFamily:"'JetBrains Mono', monospace",fontSize:10,cursor:sending[u.user]?"not-allowed":"pointer",background:sending[u.user]?"rgba(255,255,255,0.05)":"rgba(0,229,160,0.15)",border:`1px solid ${sending[u.user]?"rgba(255,255,255,0.07)":"rgba(0,229,160,0.3)"}`,color:sending[u.user]?"#2d3a52":"#00e5a0",transition:"all 0.2s"}}>{sending[u.user]?"⏳":"📤 Enviar"}</button></td>
            </tr>
          ))}</tbody>
        </table>
      </Panel>
  ...
  return (
    <div style={{padding:"24px 28px"}}>
      ...
      <Panel style={{marginBottom:20}}>
        ...
        <div style={{fontFamily:"'JetBrains Mono', monospace",fontSize:12,color:"#6b7fa3",lineHeight:1.8,marginBottom:16}}>Acesse: <strong style={{color:"#e8edf5"}}>hertzgo-vision.vercel.app → Settings → Environment Variables</strong></div>
      ...
      <Panel>
        <div style={{fontFamily:"'JetBrains Mono', monospace",fontSize:11,color:"#6b7fa3",lineHeight:2}}>
          <div>⚡ <strong style={{color:"#e8edf5"}}>HertzGo Vision v2.2</strong></div>
          <div>🏗️ Next.js 14 · React · Recharts · Z-API proxy seguro via Vercel</div>
          <div>📊 Dashboard · Semáforo · Projeção Inteligente · DRE · Usuários · Ações · Config</div>
        </div>
      </Panel>
    </div>
  );
...
export default function HertzGo() {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",minHeight:"100vh",background:"#080a0f",color:"#e8edf5",fontFamily:"'Space Grotesk', sans-serif"}}>
      ...
      <main style={{flex:1}}>
        {tab==="dash"&&<TabDashboard sessions={sessions} meta={meta} onMetaChange={setMeta} />}
