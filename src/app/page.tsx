...
function parseCSV(text: string): Session[] {
  ...
  const iEn    = idx(/^energia$/);
  const iVal   = idx(/^valor$/);
  const iStart = idx(/inicio|in.cio/);
  const iEndC  = idx(/fim do car/);
  const iEndT  = idx(/fim da trans/);
  const iDur   = idx(/dura/);
  const iSt    = idx(/recarga/);
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const startStr = g(iStart);
    const endCStr  = g(iEndC);
    const endTStr  = g(iEndT);
    ...
    sessions.push({
      date, hub: g(iLocal) || "Desconhecida", user: g(iUser) || "—",
      charger: g(iChrg), energy, value,
      duration: g(iDur), durMin: parseDurMin(g(iDur)),
      overstayMin, startHour: parseHour(startStr),
      status: g(iSt), cancelled,
    });
  }
...
function classificarUsuarios(sessions: Session[]): UserData[] {
  ...
  sessions.forEach(s => {
    if (!userMap[s.user]) userMap[s.user] = {
      user: s.user, sess: 0, kwh: 0, rev: 0,
      dates: [], hubs: [], values: [],
      isParceiro: false, isMotorista: false, isHeavy: false,
      perfil: "", perfilCor: "", localFreq: "",
    };
    const u = userMap[s.user];
    u.sess++; u.kwh += s.energy; u.rev += s.value;
    u.dates.push(s.date); u.hubs.push(s.hub); u.values.push(s.value);
  });
...
// ─── SHARED COMPONENTS ───────────────────────────────────────────────────────
function KpiCard({ label, value, sub, accent = "#00e5a0", small }: {
...
function CustomTooltip({ active, payload, label, suffix = "" }: {
  active?: boolean; payload?: {value: number; color: string}[]; label?: string; suffix?: string;
}) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 48, minHeight: "100vh" }}>
      ...
      >
        ...
        <div style={{ display: "inline-block", padding: "10px 28px", background: "#00e5a0", color: "#080a0f", borderRadius: 10, fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 13 }}>
          Selecionar arquivo
        </div>
      </div>
...
function TabDashboard({ sessions }: { sessions: Session[] }) {
  ...
  const hubs = useMemo(() => [...new Set(sessions.map(s => s.hub))].sort(), [sessions]);
  ...
  ok.forEach(s => {
    ...
    if (!byDay[k]) byDay[k] = { date: s.date, rev: 0, kwh: 0, sess: 0 };
    byDay[k].rev += s.value; byDay[k].kwh += s.energy; byDay[k].sess++;
  });
  ...
  return (
    <div style={{ padding: "24px 28px" }}>
      ...
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#6b7fa3", marginBottom: 4 }}>
        📅 {fmtDate(minDate)} → {fmtDate(maxDate)} · {days} dias · {totalSess} sessões
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 16, marginBottom: 28 }}>
        ...
        <Panel>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead><tr>
              <th style={TH}>#</th><th style={TH}>Usuário</th><th style={THR}>Sess.</th><th style={THR}>Total</th>
            </tr></thead>
            <tbody>
              {top5.map(([name, d], i) => {
                ...
                return (
                  <tr key={name}>
                    ...
                    <td style={TDR}><span style={{ background: "rgba(0,229,160,0.15)", color: "#00e5a0", padding: "2px 7px", borderRadius: 5, fontSize: 10, fontFamily: "'JetBrains Mono', monospace" }}>{d.sess}</span></td>
      ...
      <Panel>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(24,1fr)", gap: 3 }}>
          {hourData.map((h, hr) => (
            <div key={hr} title={`${hr}h: ${h.sess} sessões · ${h.kwh.toFixed(0)} kWh`} style={{ height: 36, borderRadius: 4, background: heatColor(h.sess, maxHour), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, fontFamily: "'JetBrains Mono', monospace", color: "rgba(255,255,255,0.8)", cursor: "default" }}>
              {h.sess > 0 ? h.sess : ""}
            </div>
          ))}
        </div>
            ...
            {sorted.map((u, i) => {
              ...
              return (
                <tr key={u.user} style={{ borderBottom: "1px solid rgba(255,255,255,0.02)", background: bgRow }}>
                  ...
                  <td style={TD}><span style={{ fontWeight: 500, fontSize: 12 }}>{trunc(u.user, 20)}</span>{u.isParceiro && <span style={{ fontSize: 9, marginLeft: 6, color: "#3b82f6", fontFamily: "'JetBrains Mono', monospace" }}>🔒</span>}</td>
                  <td style={{ ...TD, fontSize: 11, color: "#6b7fa3" }}>{trunc(u.localFreq, 22)}</td>
                  <td style={TDR}><span style={{ background: "rgba(0,229,160,0.15)", color: "#00e5a0", padding: "2px 7px", borderRadius: 5, fontSize: 10, fontFamily: "'JetBrains Mono', monospace" }}>{u.sess}</span></td>
                  <td style={{ ...TDR, color: "#6b7fa3" }}>{u.kwh.toFixed(1)}</td>
                  <td style={{ ...TDR, color: "#00e5a0", fontWeight: 600 }}>{brl(u.rev)}</td>
                  <td style={TDR}>
                    <div style={{ display: "flex", alignItems: "center", gap: 7, justifyContent: "flex-end" }}>
                      <span style={{ fontSize: 9, color: "#2d3a52", fontFamily: "'JetBrains Mono', monospace" }}>{pct}%</span>
...
function TabDRE({ sessions }: { sessions: Session[] }) {
  ...
  let custoEnergia  = 0;
  if (!cfg.solarProprio) {
    if (cfg.energiaTipo === "kwh")   custoEnergia = totalKwh * cfg.energiaKwh;
    if (cfg.energiaTipo === "usina") custoEnergia = cfg.usinaFixo;
  }
  ...
  if (cfg.modelo === "investidor") {
    if (restPrio > 0) { amPrio = Math.min(disp, restPrio); disp -= amPrio; }
    if (restInv  > 0) { amInv  = Math.min(disp, restInv);  disp -= amInv; }
  }
  ...
  const inp = (id: keyof DREConfig, label: string, type: "number" | "text" | "select", opts?: string[]) => (
    <div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#6b7fa3", marginBottom: 4, letterSpacing: "0.06em" }}>{label}</div>
      {type === "select" ? (
        <select value={cfg[id] as string} onChange={e => setCfg(p => ({ ...p, [id]: e.target.value }))}
          style={{ width: "100%", background: "#181d28", border: "1px solid rgba(255,255,255,0.07)", color: "#e8edf5", padding: "6px 8px", borderRadius: 8, fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }}>
          {opts?.map(o => <option key={o} value={o}>{o}</option>)}
        </select>
      ) : (
        <input type={type} min={0} value={cfg[id] as string | number}
          onChange={e => setCfg(p => ({ ...p, [id]: type === "number" ? +e.target.value : e.target.value }))}
          style={{ width: "100%", background: "#181d28", border: "1px solid rgba(255,255,255,0.07)", color: "#e8edf5", padding: "6px 8px", borderRadius: 8, fontSize: 12, fontFamily: "'JetBrains Mono', monospace" }} />
      )}
    </div>
  );
          ...
          <Panel style={{ marginBottom: 16 }}>
            ...
            {sessoes.length === 0 ? (
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>
                <thead><tr>
                  <th style={TH}>Item</th><th style={THR}>Período</th><th style={THR}>Proj. Mensal</th><th style={THR}>%</th>
                </tr></thead>
                <tbody>
                  {dreRows.map((r, i) => (
          ...
          {cfg.modelo === "investidor" && sessoes.length > 0 && (
            <Panel>
              ...
              <div style={{ background: "#181d28", borderRadius: 6, height: 22, overflow: "hidden", position: "relative", border: "1px solid rgba(255,255,255,0.07)" }}>
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontFamily: "'JetBrains Mono', monospace", color: "#fff", fontWeight: 600 }}>
                  {(pMat+pPrev+pCur).toFixed(1)}% amortizado
                </div>
              </div>
...
function TabAcoes({ sessions }: { sessions: Session[] }) {
  ...
  useEffect(() => {
    fetch("/api/zapi").then(r => r.json()).then(d => {
      setZapiStatus(d.configured && d.connected ? "ok" : "err");
    }).catch(() => setZapiStatus("err"));
  }, []);
  ...
  const enviar = async (user: string, nome: string, hub: string) => {
    ...
    try {
      ...
      setLog(p => [{ ts: new Date().toLocaleTimeString("pt-BR"), nome, status: "err", msg: "Erro de rede" }, ...p.slice(0,49)]);
    }
