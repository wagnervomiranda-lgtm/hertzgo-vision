...
import { BarChart, Bar, LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid, ReferenceLine, PieChart, Pie, Cell, RadarChart, Radar, PolarGrid, PolarAngleAxis, ScatterChart, Scatter, ZAxis } from "recharts";
...
const ESTACAO_MAP: Record<string, string> = {
  ...
  "park way": "parkway", "cidade do automovel": "cidadeauto",
  "lava jato do mamute": "mamute", "madeiro & jeronimo": "madeiro",
  "madeiro e geronimo sia brasilia": "madeiro", "madeiro & geronimo": "madeiro",
};
...
const ESTACAO_TIPO: Record<string, "propria" | "parceria" | "contratual"> = {
  costa: "parceria", parkway: "propria", cidadeauto: "propria", mamute: "contratual", madeiro: "contratual",
};
...
function defaultState(): AppState {
  return { metas: {}, dreConfigs: {}, contatos: {}, mensagens: { ...MSG_DEFAULT }, disparos: [], zapi: { instanceId: "", token: "", clientToken: "" } };
}
...
interface HealthScore {
  ...
  financeiro: number; operacional: number; investidor: number;
  diagnostico: string;
  financeiroDet: string; operacionalDet: string; investidorDet: string;
}
...
function calcHealthScore(sessions: Session[], cfg: DREConfig | null, hubK: string): HealthScore {
  ...
  if (cfg) {
    const diasNoMes = 30;
    const faturMensal = totalRev / days * diasNoMes;
    const faturAnual = faturMensal * 12;
    const aliq = cfg.modelo === "propria" ? dreSimples(faturAnual) : cfg.pctImposto;
    const impostoVal = totalRev * (aliq / 100);
    const custoEspaco = totalRev * (cfg.pctEspaco / 100);
    const custoApp = totalRev * (cfg.pctApp / 100);
    let custoEnergia = 0;
    ...
    const ll = totalRev - custoEspaco - impostoVal - custoApp - custoEnergia - fixos;
    const margem = totalRev > 0 ? (ll / totalRev) * 100 : 0;
    if (ll > 0) fin += 20;
    if (margem >= 30) fin += 20; else if (margem >= 15) fin += 10;
    finDet = `Margem ${margem.toFixed(1)}% · LL ${brl(ll)}`;
  } else {
    fin = 25;
    finDet = "Configure o DRE para análise completa";
  }
  ...
  if (cfg && cfg.modelo === "investidor") {
    ...
    const repInv = ll * (cfg.invPct / 100);
    const retMensalInv = repInv / days * diasNoMes;
  ...
  const diagnosticos = {
    saudavel: `${hubNome(hubK)} está performando bem. Mantenha o ritmo e foque em crescimento.`,
    atencao: `${hubNome(hubK)} tem pontos de melhoria. Verifique os indicadores em amarelo.`,
    critico: `${hubNome(hubK)} precisa de ação imediata. Analise o DRE detalhado.`,
  };
...
function Semaforo({ sessions }: { sessions: Session[] }) {
  ...
  const cores = {
    verde: { bg: "rgba(0,229,160,0.08)", border: "rgba(0,229,160,0.25)", dot: T.green, label: "Operação Normal", sub: "Todos os indicadores dentro das metas" },
    amarelo: { bg: "rgba(245,158,11,0.08)", border: "rgba(245,158,11,0.25)", dot: T.amber, label: "Atenção", sub: "Alguns indicadores fora da meta" },
    vermelho: { bg: "rgba(239,68,68,0.08)", border: "rgba(239,68,68,0.25)", dot: T.red, label: "Alertas Críticos", sub: "Indicadores críticos detectados" },
  };
...
function ProjecaoMensal({ sessions, meta, onMetaChange }: { sessions: Session[]; meta: number; onMetaChange: (v: number) => void }) {
  ...
  const pacingDiff = totalRev - pacingEsperado;
  const pacingPct = pacingEsperado > 0 ? (totalRev / pacingEsperado) * 100 : 0;
  ...
  const gerarInsight = (): string => {
    ...
    if (pctMeta >= 75) { if (ritmoDiff > 0) return `⚠️ Para bater a meta, precisa de R$\u00a0${ritmoNecessario.toFixed(0)}/dia nos próximos ${diasRestantes} dias — R$\u00a0${ritmoDiff.toFixed(0)}/dia a mais.`; return `⚠️ Projeção em ${pctMeta.toFixed(0)}%. Faltam R$\u00a0${faltaMeta.toFixed(0)} — ${diasRestantes} dias.`; }
    ...
    <div style={{ marginBottom: 24, background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 16, overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", borderBottom: `1px solid ${T.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ fontFamily: T.mono, fontSize: 10, color: T.text2, marginTop: 1 }}>base: {avgRevDay.toFixed(0)}/dia · {days} dias · {diasRestantes} dias restantes</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {editando ? (
            <div style={{ display: "flex", gap: 6 }}>
              <input autoFocus type="number" value={metaInput} onChange={e => setMetaInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") { onMetaChange(+metaInput || 0); setEditando(false); } if (e.key === "Escape") setEditando(false); }} style={{ width: 90, background: T.bg3, border: `1px solid ${T.green}`, color: T.text, padding: "4px 8px", borderRadius: 6, fontSize: 12, fontFamily: T.mono }} />
      ...
      {meta > 0 && (
        <div style={{ padding: "12px 20px", borderTop: `1px solid ${T.border}` }}>
          ...
          <div style={{ marginTop: 10, padding: "10px 14px", background: `${metaColor}08`, border: `1px solid ${metaColor}20`, borderRadius: 10, fontFamily: T.mono, fontSize: 11, color: T.text2, lineHeight: 1.6 }}>{gerarInsight()}</div>
        </div>
      )}
...
function UploadScreen({ onFile }: { onFile: (s: Session[]) => void }) {
  ...
  const process = useCallback(async (file: File) => {
    ...
    try {
      if (file.name.toLowerCase().match(/\.xlsx?$/)) { const { sessions } = await parseMove(file); onFile(sessions); }
      else { const text = await file.text(); onFile(parseSpott(text)); }
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 48, minHeight: "100vh" }}>
      <div style={{ textAlign: "center", marginBottom: 52 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 14, marginBottom: 16 }}>
          <div style={{ width: 52, height: 52, background: T.green, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24 }}>⚡</div>
          <div>
    ...
    <div style={{ padding: "24px 28px" }}>
          ...
          {[{ label: "⚡ DC 120kW", sess: dcSess, rev: dcRev, kwh: dcKwh, color: T.purple, bg: "rgba(139,92,246,0.08)", border: "rgba(139,92,246,0.25)" }, { label: "🔌 AC 22kW", sess: acSess, rev: acRev, kwh: acKwh, color: T.green, bg: "rgba(0,229,160,0.08)", border: "rgba(0,229,160,0.25)" }].map((dc, i) => (
            <div key={i} style={{ background: dc.bg, border: `1px solid ${dc.border}`, borderRadius: 14, padding: "14px 18px" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ fontFamily: T.sans, fontSize: 13, fontWeight: 700, color: dc.color }}>{dc.label}</span>
                <span style={{ fontFamily: T.mono, fontSize: 10, color: dc.color, padding: "2px 8px", borderRadius: 4, background: `${dc.color}20` }}>{dc.sess.length} sessões</span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8 }}>
                {[{ l: "Receita", v: brl(dc.rev) }, { l: "kWh", v: dc.kwh.toFixed(0) }, { l: "Ticket", v: brl(dc.sess.length > 0 ? dc.rev / dc.sess.length : 0) }].map((k, j) => (
                  <div key={j}><div style={{ fontFamily: T.mono, fontSize: 9, color: T.text3, marginBottom: 3 }}>{k.l}</div><div style={{ fontFamily: T.sans, fontSize: 14, fontWeight: 700, color: dc.color }}>{k.v}</div></div>
                ))}
              </div>
              <div style={{ marginTop: 10, height: 3, background: T.bg3, borderRadius: 2, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${totalRev > 0 ? (dc.rev / totalRev * 100).toFixed(0) : 0}%`, background: dc.color, borderRadius: 2 }} />
              </div>
        ...
        <Panel>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 18 }}>
            ...
            <div style={{ display: "flex", gap: 4 }}>
              {([["rev", "Receita"], ["kwh", "kWh"], ["sess", "Sessões"]] as [typeof chartMode, string][]).map(([m, l]) => (
                <button key={m} onClick={() => setChartMode(m)} style={{ padding: "3px 10px", borderRadius: 6, fontFamily: T.mono, fontSize: 10, cursor: "pointer", border: `1px solid ${chartMode === m ? T.green : T.border}`, background: chartMode === m ? T.greenDim : "transparent", color: chartMode === m ? T.green : T.text3 }}>{l}</button>
              ))}
            </div>
          </div>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={hubData} barCategoryGap="30%">
              ...
              <YAxis tickFormatter={v => chartMode === "rev" ? `R$${(v / 1000).toFixed(0)}k` : `${v}`} tick={{ fill: T.text3, fontSize: 9, fontFamily: T.mono }} axisLine={false} tickLine={false} width={52} />
              <Tooltip content={<CustomTooltip suffix={chartMode === "rev" ? "R$" : chartMode === "kwh" ? "kWh" : ""} />} cursor={{ fill: "rgba(255,255,255,0.03)" }} />
              <Bar dataKey={chartMode} fill={chartMode === "rev" ? "rgba(0,229,160,0.65)" : chartMode === "kwh" ? "rgba(245,158,11,0.65)" : "rgba(59,130,246,0.65)"} radius={[6, 6, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </Panel>
        <Panel>
      ...
      <Panel style={{ marginBottom: 28 }}>
        <ResponsiveContainer width="100%" height={200}>
          <LineChart data={dayData}>
            ...
            <YAxis yAxisId="rev" tickFormatter={v => `R$${v.toLocaleString("pt-BR", { minimumFractionDigits: 0 })}`} tick={{ fill: T.text3, fontSize: 9, fontFamily: T.mono }} axisLine={false} tickLine={false} width={68} />
            <YAxis yAxisId="sess" orientation="right" tick={{ fill: T.text3, fontSize: 9, fontFamily: T.mono }} axisLine={false} tickLine={false} width={30} />
            <Tooltip contentStyle={{ background: T.bg3, border: `1px solid ${T.border2}`, borderRadius: 10, fontFamily: T.mono, fontSize: 11 }} />
            <ReferenceLine yAxisId="rev" y={avgRev} stroke="rgba(245,158,11,0.4)" strokeDasharray="5 4" strokeWidth={1.5} />
            <Line yAxisId="rev" dataKey="rev" stroke={T.green} strokeWidth={2} dot={{ r: 2, fill: T.green }} activeDot={{ r: 5 }} name="Receita" />
            <Line yAxisId="sess" dataKey="sess" stroke={T.blue} strokeWidth={1.5} dot={false} strokeDasharray="4 3" name="Sessões" />
          </LineChart>
        </ResponsiveContainer>
      </Panel>
      ...
      <Panel>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(24,1fr)", gap: 3 }}>
          {hourData.map((h, hr) => (<div key={hr} title={`${hr}h: ${h.sess} sessões · ${h.kwh.toFixed(0)} kWh`} style={{ height: 36, borderRadius: 4, background: heatColor(h.sess, maxHour), display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "rgba(255,255,255,0.8)", cursor: "default" }}>{h.sess > 0 ? h.sess : ""}</div>))}
        </div>
...
function TabUsuarios({ sessions, appState }: { sessions: Session[]; appState: AppState }) {
  ...
  const pieData = [{ name: "Motoristas", value: motoristas.length, color: T.red }, { name: "Heavy", value: heavys.length, color: T.amber }, { name: "Shoppers", value: shoppers.length, color: "#22c55e" }, { name: "Parceiros", value: parceiros.length, color: T.blue }].filter(d => d.value > 0);
  ...
  const matrizData = [
    { x: 20, y: 85, z: motoristas.length, label: "Reter Motoristas", quadrante: "Quick Win" },
    { x: 70, y: 90, z: heavys.length, label: "Converter Heavy→Motor", quadrante: "Estratégico" },
    { x: 30, y: 50, z: shoppers.length, label: "Fidelizar Shoppers", quadrante: "Quick Win" },
    { x: 80, y: 40, z: parceiros.length, label: "Negociar Parceiros", quadrante: "Preencher" },
    { x: 15, y: 70, z: totalComTel, label: "Disparar MSG 1", quadrante: "Quick Win" },
  ];
    ...
    <div style={{ padding: "24px 28px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 16, marginBottom: 28 }}>
        ...
        <Panel>
          ...
          {[{ label: "🔴 Motoristas", rev: motoristas.reduce((a, u) => a + u.rev, 0), color: T.red }, { label: "🟡 Heavy Users", rev: heavys.reduce((a, u) => a + u.rev, 0), color: T.amber }, { label: "🟢 Shoppers", rev: shoppers.reduce((a, u) => a + u.rev, 0), color: "#22c55e" }, { label: "🔵 Parceiros", rev: parceiros.reduce((a, u) => a + u.rev, 0), color: T.blue }].map(seg => {
          ...
          <div>
            <div style={{ position: "relative", height: 280 }}>
              ...
              <div style={{ position: "absolute", inset: 0, display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "1fr 1fr", gap: 2 }}>
                {[
                  { label: "ESTRATÉGICO", sub: "Alto impacto, alto esforço", color: T.amber, pos: "top-right" },
                  { label: "QUICK WIN ⚡", sub: "Alto impacto, baixo esforço", color: T.green, pos: "top-left" },
                  { label: "PREENCHER", sub: "Baixo impacto, alto esforço", color: T.text3, pos: "bottom-right" },
                  { label: "ATENÇÃO", sub: "Baixo impacto, baixo esforço", color: T.blue, pos: "bottom-left" },
                ].map((q, i) => (
                  <div key={i} style={{ background: `${q.color}08`, border: `1px solid ${q.color}20`, borderRadius: 8, padding: "10px 12px", display: "flex", flexDirection: "column", justifyContent: i < 2 ? "flex-start" : "flex-end" }}>
                    <div style={{ fontFamily: T.mono, fontSize: 9, fontWeight: 700, color: q.color, letterSpacing: "0.08em" }}>{q.label}</div>
                    <div style={{ fontFamily: T.mono, fontSize: 8, color: T.text3, marginTop: 2 }}>{q.sub}</div>
                  </div>
                ))}
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontFamily: T.mono, fontSize: 9, color: T.text3, marginTop: 4 }}>
              <span>← Baixo Esforço</span><span>Alto Esforço →</span>
            </div>
          </div>
          <div>
            ...
            {matrizData.sort((a, b) => b.y - a.y).map((item, i) => {
              const qColor = item.quadrante === "Quick Win" ? T.green : item.quadrante === "Estratégico" ? T.amber : T.text3;
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", background: `${qColor}06`, border: `1px solid ${qColor}20`, borderRadius: 8, marginBottom: 8 }}>
                  <div style={{ width: 28, height: 28, borderRadius: "50%", background: `${qColor}20`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                    <span style={{ fontFamily: T.mono, fontSize: 10, fontWeight: 700, color: qColor }}>{i + 1}</span>
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontFamily: T.sans, fontSize: 12, fontWeight: 600, color: T.text }}>{item.label}</div>
                    <div style={{ fontFamily: T.mono, fontSize: 9, color: T.text3 }}>Impacto: {item.y}% · Esforço: {item.x}% · {item.z} usuários</div>
                  </div>
                  <span style={{ fontFamily: T.mono, fontSize: 9, padding: "2px 8px", borderRadius: 4, background: `${qColor}20`, color: qColor }}>{item.quadrante}</span>
                </div>
              );
            })}
          </div>
      ...
      {motoristas.length > 0 && (
        <>
          ...
          <div style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 12, padding: "16px 20px", marginBottom: 16 }}>
            ...
            <div style={{ fontFamily: T.mono, fontSize: 11, color: T.text2, lineHeight: 1.8, marginBottom: 12 }}>Score de 0–100 calculado automaticamente a cada CSV. Mede a saúde do relacionamento com cada motorista.</div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 8 }}>
              {[{ status: "🟠 Em Risco", score: "26–50", acao: "AGIR AGORA — frequência caindo. Enviar MSG de reengajamento.", cor: "#fb923c" }, { status: "🔴 Churn", score: "0–25", acao: "Última tentativa de resgate.", cor: T.red }, { status: "🟡 Regular", score: "51–75", acao: "Monitorar. Enviar novidade quando tiver.", cor: T.amber }, { status: "🟢 VIP Ativo", score: "76–100", acao: "Reconhecer — MSG VIP para fidelizar.", cor: T.green }].map((s, i) => (
          ...
          <Panel style={{ marginBottom: 24 }}>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 1, background: T.border, borderRadius: 10, overflow: "hidden", marginBottom: 14 }}>
              {[{ label: "🟠 Em Risco", count: motoristas.filter(u => vipScores[u.user]?.status === "em_risco").length, color: "#fb923c" }, { label: "🔴 Churn", count: motoristas.filter(u => vipScores[u.user]?.status === "churned").length, color: T.red }, { label: "🟡 Regular", count: motoristas.filter(u => vipScores[u.user]?.status === "regular").length, color: T.amber }, { label: "🟢 VIP Ativo", count: motoristas.filter(u => vipScores[u.user]?.status === "ativo").length, color: T.green }].map((s, i) => (
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead><tr><th style={TH}>Motorista</th><th style={TH}>Hub</th><th style={THR}>Score</th><th style={THR}>Freq/sem</th><th style={THR}>Dias s/ recarga</th><th style={TH}>Status</th><th style={TH}>Telefone</th></tr></thead>
              <tbody>
                {motoristasOrdenados.map(u => {
                  const v = vipScores[u.user];
                  const tel = getTel(u.user);
                  const sc = v?.status === "ativo" ? T.green : v?.status === "regular" ? T.amber : v?.status === "em_risco" ? "#fb923c" : T.red;
                  const se = v?.status === "ativo" ? "🟢" : v?.status === "regular" ? "🟡" : v?.status === "em_risco" ? "🟠" : "🔴";
                  const bg = v?.status === "em_risco" ? "rgba(251,146,60,0.04)" : v?.status === "churned" ? "rgba(239,68,68,0.05)" : "";
                  return (
                    <tr key={u.user} style={{ borderBottom: "1px solid rgba(255,255,255,0.02)", background: bg }}>
                      ...
                      <td style={TDR}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, justifyContent: "flex-end" }}>
                          <div style={{ width: 40, height: 4, background: T.bg3, borderRadius: 2, overflow: "hidden" }}>
                            <div style={{ height: "100%", width: `${v?.score || 0}%`, background: sc, borderRadius: 2 }} />
                          </div>
                          <span style={{ color: sc, fontWeight: 600, fontSize: 11 }}>{v?.score || 0}</span>
                        </div>
                      </td>
                      ...
                      <td style={TD}><span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: `${sc}20`, color: sc, fontFamily: T.mono }}>{se} {v?.status || "—"}</span></td>
      ...
      <Panel style={{ padding: 0, overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr><th style={TH}>#</th><th style={TH}>Perfil</th><th style={TH}>Usuário</th><th style={TH}>Hub</th><th style={THR}>Sess.</th><th style={THR}>kWh</th><th style={THR}>Receita</th><th style={TH}>Telefone</th></tr></thead>
          <tbody>
            {sorted.map((u, i) => {
              const tel = getTel(u.user);
...
function TabDRE({ sessions, appState }: { sessions: Session[]; appState: AppState }) {
  ...
  let ll = bruto, margem = 0, repInv = 0, repHz = bruto;
  let retMensalInv = 0, rentAnual = 0, faltaAmort = 0, mesesPay = Infinity;
  ...
  if (cfg) {
    ...
    if (!cfg.solarProprio) { if (cfg.energiaTipo === "kwh") custoEnergia = totalKwh * cfg.energiaKwh; if (cfg.energiaTipo === "usina") custoEnergia = cfg.usinaFixo; }
    const fixos = cfg.fixoInternet + cfg.fixoAluguel;
    ll = bruto - custoEspaco - impostoVal - custoApp - custoEnergia - fixos;
  ...
  // Health scores de todas as estações
  const allHealthScores = hubs.map(h => ({ hub: h, hs: calcHealthScore(sessions, appState.dreConfigs[h] || null, h) }));

  // Radar data para a estação selecionada
  const hs = calcHealthScore(sessions, cfg, station);
  const radarData = [
    { subject: "Financeiro", value: hs.financeiro, fullMark: 40 },
    { subject: "Operacional", value: hs.operacional, fullMark: 35 },
    { subject: "Investidor", value: hs.investidor, fullMark: 25 },
  ];

  return (
    <div style={{ padding: "24px 28px" }}>

      {/* Station Health Score — visão geral todas estações */}
      <SectionLabel>🏥 Station Health Score — Rede HertzGo</SectionLabel>
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(hubs.length, 5)},1fr)`, gap: 12, marginBottom: 28 }}>
        {allHealthScores.map(({ hub, hs: h }) => {
          ...
          return (
            <div key={hub} onClick={() => setStation(hub)} style={{ background: `${color}06`, border: `1px solid ${color}${station === hub ? "60" : "25"}`, borderRadius: 14, padding: "16px", cursor: "pointer", transition: "all 0.2s", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: color }} />
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ fontFamily: T.sans, fontSize: 12, fontWeight: 700, color: T.text }}>{hubNome(hub)}</span>
                <span style={{ fontSize: 9, padding: "1px 6px", borderRadius: 4, background: `${color}20`, color, fontFamily: T.mono }}>{tipo}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                <span style={{ fontSize: 24 }}>{emoji}</span>
                <div>
                  <div style={{ fontFamily: T.sans, fontSize: 28, fontWeight: 800, color, lineHeight: 1 }}>{h.total}</div>
                  <div style={{ fontFamily: T.mono, fontSize: 9, color: T.text3 }}>/ 100</div>
                </div>
              </div>
              <div style={{ height: 4, background: T.bg3, borderRadius: 2, overflow: "hidden", marginBottom: 8 }}>
                <div style={{ height: "100%", width: `${h.total}%`, background: color, borderRadius: 2, transition: "width 0.8s ease" }} />
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 4 }}>
                {[{ l: "💰", v: h.financeiro, max: 40 }, { l: "⚡", v: h.operacional, max: 35 }, { l: "🤝", v: h.investidor, max: 25 }].map((d, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ fontFamily: T.mono, fontSize: 8, color: T.text3 }}>{d.l}</div>
                    <div style={{ fontFamily: T.mono, fontSize: 10, fontWeight: 700, color: d.v / d.max >= 0.7 ? T.green : d.v / d.max >= 0.4 ? T.amber : T.red }}>{d.v}/{d.max}</div>
                  </div>
                ))}
              </div>
              <div style={{ fontFamily: T.mono, fontSize: 9, color: T.text2, marginTop: 8, lineHeight: 1.4 }}>{h.diagnostico}</div>
            </div>
          );
        })}
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {hubs.map(h => (
            <button key={h} onClick={() => setStation(h)} style={{ padding: "4px 12px", borderRadius: 8, fontFamily: T.mono, fontSize: 10, cursor: "pointer", border: `1px solid ${station === h ? T.green : T.border}`, background: station === h ? T.greenDim : "transparent", color: station === h ? T.green : T.text2 }}>
              {hubNome(h)}
            </button>
          ))}
        </div>
      ...
      {!cfg && (
        <div style={{ background: "rgba(245,158,11,0.08)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 12, padding: "14px 18px", marginBottom: 20, fontFamily: T.mono, fontSize: 12, color: T.amber }}>
          ⚙️ Configure o DRE desta estação em <strong>Config → DRE Config</strong> para ver análise financeira completa.
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: cfg ? "1fr 1fr" : "1fr", gap: 20 }}>
        <div>
          {sessoes.length === 0 ? (
            <Panel><div style={{ fontFamily: T.mono, fontSize: 12, color: T.text3, padding: "24px 0", textAlign: "center" }}>Nenhuma sessão encontrada para {hubNome(station)}.</div></Panel>
          ) : cfg ? (
            <Panel>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: T.mono, fontSize: 12 }}>
                ...
                <tbody>
                  {dreRows.map((r, i) => (
                    <tr key={i} style={{ borderTop: r.sep ? `1px solid ${T.border}` : "none", borderBottom: "1px solid rgba(255,255,255,0.02)" }}>
                      <td style={{ ...TD, fontWeight: r.bold ? 700 : 400, color: r.accent || (r.val >= 0 ? T.text : T.red) }}>{r.label}</td>
                      <td style={{ ...TDR, color: r.accent || (r.val >= 0 ? T.green : T.red), fontWeight: r.bold ? 700 : 400 }}>{brl(r.val)}</td>
                      <td style={{ ...TDR, color: T.text2 }}>{brl(r.val * (diasNoMes / periodDays))}</td>
                      <td style={{ ...TDR, color: T.text3 }}>{bruto > 0 ? `${(Math.abs(r.val) / bruto * 100).toFixed(1)}%` : "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <Panel>
              <div style={{ fontFamily: T.sans, fontSize: 14, fontWeight: 600, marginBottom: 14, color: T.text }}>Dados do Período</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 10 }}>
                <KpiCard label="Receita Bruta" value={brl(bruto)} accent={T.green} small />
                <KpiCard label="kWh Total" value={`${totalKwh.toFixed(0)} kWh`} accent={T.amber} small />
                <KpiCard label="Sessões" value={`${sessoes.length}`} accent={T.blue} small />
                <KpiCard label="Preço/kWh" value={`R$\u00a0${(totalKwh > 0 ? bruto / totalKwh : 0).toFixed(2)}`} accent={T.purple} small />
              </div>
            </Panel>
          )}
        </div>
          ...
          <div>
            ...
            <Panel style={{ marginBottom: 16 }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <div style={{ fontFamily: T.sans, fontSize: 14, fontWeight: 600, color: T.text }}>🏥 Health Score — {hubNome(station)}</div>
                <div style={{ fontFamily: T.sans, fontSize: 22, fontWeight: 800, color: hs.status === "saudavel" ? T.green : hs.status === "atencao" ? T.amber : T.red }}>{hs.total}/100</div>
              </div>
              <ResponsiveContainer width="100%" height={180}>
                <RadarChart data={radarData}>
                  <PolarGrid stroke={T.border} />
                  <PolarAngleAxis dataKey="subject" tick={{ fill: T.text2, fontSize: 11, fontFamily: T.mono }} />
                  <Radar name="Score" dataKey="value" stroke={hs.status === "saudavel" ? T.green : hs.status === "atencao" ? T.amber : T.red} fill={hs.status === "saudavel" ? T.green : hs.status === "atencao" ? T.amber : T.red} fillOpacity={0.2} />
                </RadarChart>
              </ResponsiveContainer>
              <div style={{ fontFamily: T.mono, fontSize: 11, color: T.text2, padding: "8px 10px", background: "rgba(255,255,255,0.03)", borderRadius: 8 }}>{hs.diagnostico}</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 8, marginTop: 10 }}>
                {[{ l: "💰 Financeiro", v: hs.financeiroDet }, { l: "⚡ Operacional", v: hs.operacionalDet }, { l: "🤝 Investidor", v: hs.investidorDet }].map((d, i) => (
                  <div key={i} style={{ background: T.bg3, borderRadius: 8, padding: "8px 10px" }}>
                    <div style={{ fontFamily: T.mono, fontSize: 9, color: T.text2, marginBottom: 3 }}>{d.l}</div>
                    <div style={{ fontFamily: T.mono, fontSize: 9, color: T.text3, lineHeight: 1.4 }}>{d.v}</div>
                  </div>
                ))}
              </div>
            </Panel>
...
function TabAcoes({ sessions, appState, onSaveDisparos }: { sessions: Session[]; appState: AppState; onSaveDisparos: (d: AppState["disparos"]) => void }) {
  ...
  const enviarLote = async () => {
    ...
    if (!lista.length) { alert("Nenhum selecionado com telefone."); return; }
    if (!confirm(`Disparar MSG 1 para ${lista.length} usuários? Delay de 3s entre envios.`)) return;
    setEnviandoLote(true);
    for (let i = 0; i < lista.length; i++) { await enviarUm(lista[i].user, lista[i].localFreqKey); if (i < lista.length - 1) await new Promise(r => setTimeout(r, 3000)); }
    setSelecionados(new Set()); setEnviandoLote(false);
  };
  ...
  const toggleTodos = () => { const comTel = leads.filter(u => getTel(u.user)).map(u => u.user); setSelecionados(prev => prev.size === comTel.length ? new Set() : new Set(comTel)); };
  return (
    <div style={{ padding: "24px 28px" }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 28 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10, padding: "10px 14px", background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button onClick={enviarLote} disabled={enviandoLote || selecionados.size === 0} style={{ padding: "6px 16px", borderRadius: 8, fontFamily: T.mono, fontSize: 11, cursor: selecionados.size === 0 ? "not-allowed" : "pointer", background: selecionados.size > 0 ? "rgba(0,229,160,0.15)" : "rgba(255,255,255,0.04)", border: `1px solid ${selecionados.size > 0 ? "rgba(0,229,160,0.3)" : T.border}`, color: selecionados.size > 0 ? T.green : T.text3, transition: "all 0.2s", opacity: enviandoLote ? 0.6 : 1 }}>
      ...
      <Panel style={{ padding: 0, overflow: "hidden", marginBottom: 24 }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead><tr><th style={{ ...TH, width: 20 }}></th><th style={TH}>Perfil</th><th style={TH}>Usuário</th><th style={TH}>Hub</th><th style={THR}>kWh</th><th style={THR}>Receita</th><th style={TH}>Telefone</th><th style={THR}>Ação</th></tr></thead>
          <tbody>
            ...
            {leads.slice(0, 50).map(u => {
              const tel = getTel(u.user), sel = selecionados.has(u.user);
              return (
                <tr key={u.user} style={{ borderBottom: "1px solid rgba(255,255,255,0.02)", background: sel ? "rgba(0,229,160,0.04)" : "" }}>
                  <td style={{ ...TD, width: 20, textAlign: "center" }}>{tel && <input type="checkbox" checked={sel} onChange={() => toggleSel(u.user)} style={{ accentColor: T.green, width: 13, height: 13, cursor: "pointer" }} />}</td>
                  <td style={TD}><span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, background: `${u.perfilCor}20`, color: u.perfilCor, fontFamily: T.mono }}>{u.perfil}</span></td>
                  ...
                  <td style={TDR}>{tel ? (<button onClick={() => enviarUm(u.user, u.localFreqKey)} disabled={sending[u.user]} style={{ padding: "4px 12px", borderRadius: 6, fontFamily: T.mono, fontSize: 10, cursor: sending[u.user] ? "not-allowed" : "pointer", background: sending[u.user] ? "rgba(255,255,255,0.05)" : T.greenDim, border: `1px solid ${sending[u.user] ? T.border : "rgba(0,229,160,0.3)"}`, color: sending[u.user] ? T.text3 : T.green, transition: "all 0.2s" }}>{sending[u.user] ? "⏳" : "📤 MSG 1"}</button>) : <span style={{ color: T.text3, fontSize: 10, fontFamily: T.mono }}>sem tel</span>}</td>
                </tr>
              );
            })}
          </tbody>
...
function TabConfig({ appState, onSave }: { appState: AppState; onSave: (partial: Partial<AppState>) => void }) {
  const [activeSection, setActiveSection] = useState<"contatos" | "mensagens" | "dre" | "zapi">("contatos");
  ...
  const dreDefaults: Record<string, Partial<DREConfig>> = {
    costa: { modelo: "investidor", pctEspaco: 50, invNome: "FL BR SOLUÇÕES SUSTENTÁVEIS LTDA", invPct: 50, invTotal: 150000, invPago: 100000, solarProprio: true },
    parkway: { modelo: "propria", pctEspaco: 0, propriaInstalacao: 100000 },
    cidadeauto: { modelo: "propria", pctEspaco: 0, propriaInstalacao: 100000 },
    mamute: { modelo: "investidor", pctEspaco: 50 },
    madeiro: { modelo: "investidor", pctEspaco: 50 },
  };
  ...
  const handleContactUpload = async (file: File) => {
    try {
      ...
      setUploadStatus(`✅ ${dados.length} usuários · ${comTel} com telefone · Estação: ${hubNome(estacaoKey)}`);
    } catch (e) { setUploadStatus(`❌ Erro: ${(e as Error).message}`); }
  };

  const testarZapi = async () => {
    setZapiTesting(true); setZapiTestResult("");
    try { const r = await fetch("/api/zapi"); const d = await r.json(); setZapiTestResult(d.connected ? "✅ Conectada e funcionando" : d.configured ? "⚠️ Configurada mas desconectada" : "❌ Não configurada"); }
  ...
  const estacoesDRE = [
    { key: "costa", nome: "Costa Atacadão", tipo: "Parceria" },
    { key: "parkway", nome: "Park Way", tipo: "Própria" },
    { key: "cidadeauto", nome: "Cidade do Automóvel", tipo: "Própria" },
    { key: "mamute", nome: "Lava Jato do Mamute", tipo: "Contratual" },
    { key: "madeiro", nome: "Madeiro & Jerônimo", tipo: "Contratual" },
  ];

  return (
    <div style={{ padding: "24px 28px" }}>
      <div style={{ display: "flex", gap: 8, marginBottom: 24 }}>
        {([["contatos", "📱 Contatos"], ["mensagens", "✉️ Mensagens"], ["dre", "💼 DRE Config"], ["zapi", "🔌 Z-API"]] as [string, string][]).map(([id, label]) => (
      ...
      {activeSection === "contatos" && (
        <>
          <div style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 10, padding: "12px 16px", fontFamily: T.mono, fontSize: 11, color: "#93c5fd", marginBottom: 20 }}>
            ℹ️ Importe o CSV de usuários. A estação é detectada automaticamente. Faça 1x por semana.
          </div>
          <div style={{ background: T.bg2, border: `1px solid ${T.border}`, borderRadius: 14, padding: "24px", marginBottom: 20, textAlign: "center" }}>
            ...
            <div style={{ fontFamily: T.mono, fontSize: 11, color: T.text2, marginBottom: 20 }}>Estação detectada automaticamente pelo nome do arquivo ou conteúdo</div>
        ...
        <>
          ...
          {msgFields.map(([key, label]) => (
            <div key={key} style={{ marginBottom: 16 }}>
              ...
              <textarea value={msgs[key]} onChange={e => setMsgs(p => ({ ...p, [key]: e.target.value }))} style={{ width: "100%", background: T.bg3, border: `1px solid ${T.border}`, color: T.text, padding: "10px 12px", borderRadius: 10, fontSize: 12, fontFamily: T.mono, resize: "vertical", minHeight: key.startsWith("cupom") ? 40 : 90, lineHeight: 1.6 }} />
            </div>
          ))}
      ...
      {activeSection === "dre" && (
        <>
          <div style={{ background: "rgba(0,229,160,0.06)", border: "1px solid rgba(0,229,160,0.2)", borderRadius: 10, padding: "12px 16px", fontFamily: T.mono, fontSize: 11, color: T.green, marginBottom: 20 }}>
            ℹ️ Configure uma vez e salve. Os dados pré-carregam com os valores padrão de cada estação. Só altere quando houver mudança contratual.
          </div>
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            {estacoesDRE.map(e => (
              <button key={e.key} onClick={() => setDreStation(e.key)} style={{ padding: "6px 14px", borderRadius: 8, fontFamily: T.mono, fontSize: 11, cursor: "pointer", border: `1px solid ${dreStation === e.key ? T.green : T.border}`, background: dreStation === e.key ? T.greenDim : "transparent", color: dreStation === e.key ? T.green : T.text2, transition: "all 0.2s" }}>
                {e.nome} <span style={{ fontSize: 9, opacity: 0.7 }}>({e.tipo})</span>
              </button>
            ))}
          </div>
          <Panel>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button onClick={() => { onSave({ dreConfigs: { ...appState.dreConfigs, [dreStation]: cfg } }); setDreSaved(true); setTimeout(() => setDreSaved(false), 2000); }} style={{ background: dreSaved ? "rgba(0,229,160,0.2)" : T.greenDim, border: `1px solid ${dreSaved ? T.green : "rgba(0,229,160,0.3)"}`, color: T.green, padding: "6px 18px", borderRadius: 8, fontSize: 11, cursor: "pointer", fontFamily: T.mono, transition: "all 0.3s" }}>
                {dreSaved ? "✅ Salvo" : "💾 Salvar"}
              </button>
            </div>
            ...
            {cfg.modelo === "investidor" && (
              <>
                <div style={{ fontFamily: T.mono, fontSize: 9, color: T.text3, letterSpacing: "0.12em", textTransform: "uppercase" as const, margin: "16px 0 12px", borderTop: `1px solid ${T.border}`, paddingTop: 14 }}>Investidor / Split</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                  {inp("invNome", "Nome Investidor", "text")} {inp("invPct", "% Investidor do LL", "number")} {inp("invTotal", "Investimento Total", "number")}
                  {inp("invPago", "Já Investido", "number")} {inp("invDividaPrio", "Dívida Prioritária", "number")} {inp("invAmort", "Já Amortizado", "number")}
                </div>
              </>
            )}
            {cfg.modelo === "propria" && (
              <>
                <div style={{ fontFamily: T.mono, fontSize: 9, color: T.text3, letterSpacing: "0.12em", textTransform: "uppercase" as const, margin: "16px 0 12px", borderTop: `1px solid ${T.border}`, paddingTop: 14 }}>Loja Própria</div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
                  {inp("propriaInstalacao", "Custo Instalação", "number")} {inp("propriaAmort", "Já Amortizado", "number")}
                </div>
              </>
            )}
            <label style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 16, cursor: "pointer", fontFamily: T.mono, fontSize: 11, color: T.text2 }}>
      ...
      {activeSection === "zapi" && (
        <Panel>
          ...
          <div style={{ background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.2)", borderRadius: 10, padding: "12px 14px", fontFamily: T.mono, fontSize: 11, color: "#93c5fd" }}>
            ℹ️ Credenciais salvas no browser. Para segurança máxima, configure também as variáveis de ambiente no Vercel.
          </div>
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${T.border}`, fontFamily: T.mono, fontSize: 11, color: T.text2, lineHeight: 2 }}>
            <div>⚡ <strong style={{ color: T.text }}>HertzGo Vision v3.2</strong></div>
            <div>📊 Dashboard · DC/AC · Health Score · DRE Config · Matriz Priorização · VIP Score · CRM · Move XLSX</div>
          </div>
        </Panel>
      )}
    </div>
